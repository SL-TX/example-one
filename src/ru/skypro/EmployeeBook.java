...
public class EmployeeBook {
    ...
    public Employee getEmployeeWithMinSalary() {
        return Arrays.stream(eployies)
                .reduce((e1, e2) -> e1.getSalary() < e2.getSalary() ? e1 : e2).orElseThrow();
    }
    ...
    public String listNamesOfAllEmployies() {
        return "[" + Arrays.stream(eployies)
                .map(Employee::toStringOnlyFIO)
                .collect(Collectors.joining(",\n")) + "]";
    }

    public void salaryIndex(Integer percent) {
        Arrays.stream(eployies)
                .forEach((e) -> e.setSalary(e.getSalary() + e.getSalary() * percent / 100));
    }
    ...
    public Double getAvgSalariesInDepartment(String department) {
        return (double) getSumSalariesInDepartment(department) /
                Arrays.stream(eployies)
                        .filter(e -> Objects.equals(e.getDepartment(), department))
                        .count();
    }
    ...
    public String listNamesOfEmployiesInDepartment(String department) {
        return "[" + Arrays.stream(eployies)
                .filter(e -> Objects.equals(e.getDepartment(), department))
                .map(Employee::toStringWithoutDepartment)
                .collect(Collectors.joining(",\n")) + "]";
    }

    public Employee[] listEmployiesWithSalaryMoreThanOrEqual(Integer value) {
        var list = Arrays.stream(eployies)
                .filter(e -> e.getSalary() >= value).toList();
    ...
    public void delEmployeeByFIO(String firstName, String lastName, String secondName) {
        for (var i = 0; i < 10; i++) {
            if (eployies[i] != null &&
                    Objects.equals(eployies[i].getFirstName(), firstName) &&
    ...
    public void setEmployeeSalaryAndDepartment(String firstName, String lastName, String secondName, String department, Integer salary) {
        for (var i = 0; i < 10; i++) {
            if (eployies[i] != null &&
                    Objects.equals(eployies[i].getFirstName(), firstName) &&
                    Objects.equals(eployies[i].getLastName(), lastName) &&
                ...
                if (salary != null)
                    eployies[i].setSalary(salary);
                if (department != null)
                    eployies[i].setDepartment(department);
            }
        }
    }
